...
class VoiceReader:
    ...
    def get_audio_path(self, text: str, voice: str = DEFAULT_VOICE, language: Optional[str] = None) -> Path:
        ...
        if voice not in AVAILABLE_VOICES:
            raise ValueError(f"Invalid voice. Choose from: {', '.join(AVAILABLE_VOICES)}")
        ...
        try:
            ...
            if os.name == "posix":  # macOS, Linux
                if "darwin" in os.uname().sysname.lower():  # macOS
                    subprocess.run(["afplay", str(audio_path)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                else:  # Linux
                    try:
                        subprocess.run(["mpg123", str(audio_path)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    except FileNotFoundError:
                        subprocess.run(["aplay", str(audio_path)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def play_audio(self, audio_path: Path) -> None:
        ...
        self.active_playback_threads = [t for t in self.active_playback_threads if t.is_alive()]
    ...
    def speak(self, text: str, voice: str = DEFAULT_VOICE, language: Optional[str] = None) -> None:
